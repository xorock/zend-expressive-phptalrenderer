...
class PhptalEngineFactory
{
    ...
    public function __invoke(ContainerInterface $container)
    {
        ...
        $helpers = (isset($config['helpers']) && is_array($config['helpers']))
            ? $config['helpers']
            : [];
        
        $this->injectHelpers($engine, $container, $helpers);
     ...
     */
    private function injectHelpers(PhptalEngine $template, ContainerInterface $container, array $helpers)
    {        
        ...
        foreach ($helpers as $helper) {
            $helper = $helperManager->loadHelper($helper, $container);
            if (!$helperManager->hasHelper($helper->getHelperName())) {
                $helperManager->registerHelper($helper);
            }
        }
